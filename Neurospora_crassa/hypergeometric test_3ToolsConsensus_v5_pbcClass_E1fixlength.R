...
pbcs=read.table("neurospora_PBC_genes.xls",sep="\t",head=T)
...
TFs2=StatisValues
colnames(TFs2)=c("JGI_geneID","Regulon_genes","PBC_genes","Cazy_genes","metabolism_genes","transporter_genes","regulator_genes","starch_genes","cellulose_genes","mannan_genes","xyloglucan_genes","xylan_genes","pectin_genes","galacturonicAcid_genes","rhamnose_genes","PCP_genes","galactose_genes","glycerol_genes","PPP_genes","glycolysis_genes","TCA_genes",
...
## write.table(TFs2,file="Neurospora_pvalues_3toolsCensus_allTFs.xls",sep="\t",col.names=NA);


sigs=c();
for (k in 22:ncol(TFs2)){
...
mutant=read.table("mutant_genes.xls",head=T,sep="\t");
TFs4=merge(mutant,TFs3,by.x="JGI_geneID",by.y="JGI_geneID",all.y=TRUE)

write.table(TFs4,file="Neurospora_pvalues_3toolsCensus_E1fixLength_sig0.05.xls",sep="\t",col.names=NA);



t2=merge(mutant,TFs2,by.x="JGI_geneID",by.y="JGI_geneID",all.y=TRUE);
phenotype=read.table("phenotype_data_TF_mutants.xls",head=T,sep="\t");

r2=merge(phenotype,t2,by.x="GeneID",by.y="JGI_geneID",all.y=TRUE)
write.table(r2,file="Neurospora_pvalues_3toolsCensus_allTFs_withAnnotation.xls",sep="\t",col.names=NA);
